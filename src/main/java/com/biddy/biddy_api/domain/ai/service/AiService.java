...
public class AiService {
     ...
     */
    public AutoWriteDto.AutoWriteResponse generateAutoContent(List<MultipartFile> images) {
        try {
            // 이미지를 Media 객체로 변환
    ...
    private String buildAutoWritePrompt() {
        return """
                이미지를 보고 상품 정보를 JSON으로 작성해주세요.
                 카테고리는 다음 중 하나여야 합니다: ELECTRONICS, INTERIOR, BOOK, FASHION, BEAUTY, WATCH, LIFESTYLE, PET, GIFT, FOOD
    ...
    private AutoWriteDto.AutoWriteResponse parseAutoWriteResponse(String aiResponse) {
        try {
            ...
            ProductCategory detectedCategory = null;
            String categoryStr = jsonNode.path("detectedCategory").asText("");
            if (!categoryStr.isEmpty()) {
                try {
                    detectedCategory = ProductCategory.valueOf(categoryStr.toUpperCase());
                } catch (IllegalArgumentException e) {
                    log.warn("AI가 반환한 카테고리를 인식할 수 없습니다: {}", categoryStr);
                    // 기본값으로 LIFESTYLE 설정
                    detectedCategory = ProductCategory.LIFESTYLE;
                }
            }
            ...
            return AutoWriteDto.AutoWriteResponse.builder()
                    .title(title)
